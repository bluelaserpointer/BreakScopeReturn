...
public static class CharacterControllerExtension
{
    ...
    {
        bool grounded = false;
        foreach (var groundCollider in Physics.OverlapSphere(characterController.transform.position + Vector3.up * ((-characterController.height + characterController.radius) / 2 - depth), characterController.radius))
        {
