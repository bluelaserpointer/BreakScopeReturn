...
public class GunInventory : MonoBehaviour {
	...
	public Animator HandsAnimator => currentHandsAnimator;
	...
	*/
	...
	IEnumerator Spawn(int _redniBroj){
        ...
        currentGun = Instantiate(Resources.Load<GunScript>(gunsIHave[_redniBroj]), transform.position, Quaternion.identity);
		currentGun.transform.SetParent(GameManager.Instance.CurrentStage.transform);
	...
	*/
	void AssignHandsAnimator(GunScript _currentGun){
		if(_currentGun.name.Contains("Gun")){
			currentHandsAnimator = currentGun.handsAnimator;
		}
	}
